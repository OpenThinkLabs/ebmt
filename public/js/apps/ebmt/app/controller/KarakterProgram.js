Ext.define('ebmt.controller.KarakterProgram', {
    extend: 'ebmt.controller.Base',
    stores: [
             'ebmt.store.master.KarakterPrograms'
    ],
    views: [
              'master.karakterprogram.List',
              //'penerimamanfaat.individu.edit.Form',
              //'penerimamanfaat.individu.edit.Window'
    ],
    refs: [
    {
      ref: 'KarakterProgramList',
      selector: '[xtype=master.karakterprogram.list]'
    },
    //{
   ...
   // }    
    ...
    init: function() {        
    	this.listen({
    		controller: {},
    		component: {
    			'grid[xtype=master.karakterprogram.list]': {
   			      beforerender: this.loadRecords,
   			      itemdblclick: this.edit,
   			      itemcontextmenu: this.showContextMenu
 			    }, 
 			    'grid[xtype=master.karakterprogram.list] button#tambah': {
 			       click: this.add	
 			     },
 			    //'window[xtype=master.karakterprogram.list.edit.window] button#simpan':{
 			     //  click: this.save
 			    //},
    ...
    add: function( button, e, eOpts ) {
        var me = this,
            record = Ext.create( 'ebmt.model.master.KarakterProgram' );
        // show window
        me.showEditWindow( record );
    },
    ...
    save: function( button, e, eOpts ) {
        var me = this,
            grid = me.getKarakterProgramList(),
            store = grid.getStore(),
        ...
        Ext.getBody().mask( 'Simpan Data Karakter Program...' );
        // if new record...
        if( record.phantom ) {
    ...
    remove: function( record ) {
    	...
    	Ext.Msg.confirm( 'Perhatian', 'Anda yakin ingin menghapus Karakter Program ini ? Aksi ini tidak dapat dibatalkan.', function( buttonId, text, opt ) {
    		if( buttonId=='yes' ) {
    			store.remove( record );
    			store.sync({
    				/**
    ...
    showEditWindow: function( record ) {
        ...
        if( !win ) {
            win = Ext.widget( 'master.karakterprogram.edit.window', {
                title: isNew ? 'Tambah KarakterProgram Baru' : 'Edit KarakterProgram'
            });
        }
        // show window
        win.show();
