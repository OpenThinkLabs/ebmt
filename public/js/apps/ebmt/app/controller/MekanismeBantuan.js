Ext.define('ebmt.controller.MekanismeBantuan', {
    extend: 'ebmt.controller.Base',
    stores: [
             'ebmt.store.master.MekanismeBantuans'
    ],
    views: [
              'master.mekanismebantuan.List',
              //'penerimamanfaat.individu.edit.Form',
              //'penerimamanfaat.individu.edit.Window'
    ],
    refs: [
    {
      ref: 'MekanismeBantuanList',
      selector: '[xtype=master.mekanismebantuan.list]'
    },
    //{
   ...
   // }    
    ...
    init: function() {        
    	this.listen({
    		controller: {},
    		component: {
    			'grid[xtype=master.mekanismebantuan.list]': {
   			      beforerender: this.loadRecords,
   			      itemdblclick: this.edit,
   			      itemcontextmenu: this.showContextMenu
 			    }, 
 			    'grid[xtype=master.mekanismebantuan.list] button#tambah': {
 			       click: this.add	
 			     },
    ...
    add: function( button, e, eOpts ) {
        var me = this,
            record = Ext.create( 'ebmt.model.master.MekanismeBantuan' );
        // show window
        me.showEditWindow( record );
    },
    ...
    save: function( button, e, eOpts ) {
        ...
        Ext.getBody().mask( 'Simpan Data Mekanisme Bantuan...' );
        // if new record...
        if( record.phantom ) {
    ...
    remove: function( record ) {
    	...
    	Ext.Msg.confirm( 'Perhatian', 'Anda yakin ingin menghapus Mekanisme Bantuan ini ? Aksi ini tidak dapat dibatalkan.', function( buttonId, text, opt ) {
    		if( buttonId=='yes' ) {
    			store.remove( record );
    			store.sync({
    				/**
    ...
    showEditWindow: function( record ) {
        ...
        if( !win ) {
            win = Ext.widget( 'master.mekanismebantuan.edit.window', {
                title: isNew ? 'Tambah Mekanisme Bantuan Baru' : 'Edit Mekanisme Bantuan'
            });
        }
        // show window
        win.show();
