Ext.define('ebmt.controller.Perihal', {
    extend: 'ebmt.controller.Base',
    stores: [
             'ebmt.store.master.Perihals'
    ],
    views: [
              'master.perihal.List',
    ...
    {
      ref: 'PerihalList',
      selector: '[xtype=master.perihal.list]'
    },
    //{
   ...
   // }    
    ...
    init: function() {        
    	this.listen({
    		controller: {},
    		component: {
    			'grid[xtype=master.perihal.list]': {
   			      beforerender: this.loadRecords,
   			      itemdblclick: this.edit,
   			      itemcontextmenu: this.showContextMenu
 			    }, 
 			    'grid[xtype=master.perihal.list] button#tambah': {
 			       click: this.add	
 			     },
    ...
    add: function( button, e, eOpts ) {
        var me = this,
            record = Ext.create( 'ebmt.model.master.Perihal' );
        // show window
        me.showEditWindow( record );
    },
    ...
    save: function( button, e, eOpts ) {
        ...
        Ext.getBody().mask( 'Simpan Data Perihal...' );
        // if new record...
        if( record.phantom ) {
    ...
    remove: function( record ) {
    	...
    	Ext.Msg.confirm( 'Perhatian', 'Anda yakin ingin menghapus Perihal ini ? Aksi ini tidak dapat dibatalkan.', function( buttonId, text, opt ) {
    		if( buttonId=='yes' ) {
    			store.remove( record );
    			store.sync({
    				/**
    ...
    showEditWindow: function( record ) {
        ...
        if( !win ) {
            win = Ext.widget( 'master.perihal.edit.window', {
                title: isNew ? 'Tambah Perihal Baru' : 'Edit Perihal'
            });
        }
        // show window
        win.show();
