Ext.define('ebmt.controller.Asnaf', {
    extend: 'ebmt.controller.Base',
    stores: [
             'ebmt.store.master.Asnafs'
    ],
    views: [
              'master.asnaf.List',
    ...
    {
      ref: 'AsnafList',
      selector: '[xtype=master.asnaf.list]'
    },
    //{
   ...
   // }    
    ...
    init: function() {        
    	this.listen({
    		controller: {},
    		component: {
    			'grid[xtype=master.asnaf.list]': {
   			      beforerender: this.loadRecords,
   			      itemdblclick: this.edit,
   			      itemcontextmenu: this.showContextMenu
 			    }, 
 			    'grid[xtype=master.asnaf.list] button#tambah': {
 			       click: this.add	
 			     },
 			    //'window[xtype=master.asnaf.list.edit.window] button#simpan':{
 			     //  click: this.save
 			    //},
    ...
    add: function( button, e, eOpts ) {
        var me = this,
            record = Ext.create( 'ebmt.model.master.Asnaf' );
        // show window
        me.showEditWindow( record );
    },
    ...
    save: function( button, e, eOpts ) {
        ...
        Ext.getBody().mask( 'Simpan Data Asnaf...' );
        // if new record...
        if( record.phantom ) {
    ...
    remove: function( record ) {
    	...
    	Ext.Msg.confirm( 'Perhatian', 'Anda yakin ingin menghapus Asnaf ini ? Aksi ini tidak dapat dibatalkan.', function( buttonId, text, opt ) {
    		if( buttonId=='yes' ) {
    			store.remove( record );
    			store.sync({
    				/**
    ...
    showEditWindow: function( record ) {
        ...
        if( !win ) {
            win = Ext.widget( 'master.asnaf.edit.window', {
                title: isNew ? 'Tambah Asnaf Baru' : 'Edit Asnaf'
            });
        }
        // show window
        win.show();
