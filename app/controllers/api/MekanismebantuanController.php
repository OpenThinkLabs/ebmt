...
use Ebmt\Models\MMekanismeBantuan as MMekanismeBantuan ;

class MekanismeBantuanController extends ControllerBase
{
    ...
    public function indexAction()
    {
        ...
        $this->setJsonResponse() ;
        	...
        	->columns('id, mekanisme_bantuan, status')
        	->from('Ebmt\Models\MMekanismeBantuan') 
        	->where("1=1") ;
        	...
        	$mekanisme_bantuan_arr          = array();
        	$mekanisme_bantuan_arr["total"] = $page->total_items; 
        	
        	foreach($page->items as $item) {
        		$mekanisme_bantuan_arr["data"][] = array("id"=>$item->id,
        		                                        "mekanisme_bantuan"=>$item->mekanisme_bantuan, 
        				                                "status"=>$item->status) ; 
        	}
        	
        	 return $mekanisme_bantuan_arr ;
    }
