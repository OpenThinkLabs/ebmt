...
class AsnafController extends ControllerBase
{
    ...
    public function indexAction()
    {
        ...
        $this->setJsonResponse() ;
        	...
        	->columns('id, asnaf, status')
        	->from('Ebmt\Models\MAsnaf') 
        	->where("1=1") ;
        	...
        	$asnaf_arr          = array();
        	$asnaf_arr["total"] = $page->total_items; 
        	
        	foreach($page->items as $item) {
        		$asnaf_arr["data"][] = array("id"=>$item->id,
        		                             "asnaf"=>$item->asnaf, 
        				                     "status"=>$item->status) ; 
        	}
